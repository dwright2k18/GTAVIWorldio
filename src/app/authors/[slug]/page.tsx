...
export async function generateMetadata({
  ...
  return {
    ...
    description: author.author.bio ?? `Articles by ${author.author.name}.`,
    alternates: { canonical: `/authors/${author.author.slug}` },
  };
}

export default async function AuthorPage({ params }: AuthorPageProps) {
  const record = await getPublicAuthor((await params).slug);
  if (!record) notFound();

  const { author } = record;
  const personSchema = author.name.endsWith("Staff")
    ? null
    : {
    ...
    <main className="min-h-screen py-10 sm:py-14">
      ...
      <div className="site-shell">
        ...
        <header className="mt-10 max-w-3xl">
          ...
          {author.role ? (
            <p className="mt-3 font-bold text-cyan-300">{author.role}</p>
          ) : null}
          {author.bio ? (
        ...
        <section className="mt-12">
          <h2 className="text-3xl font-black text-white">Article history</h2>
