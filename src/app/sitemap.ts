...
import { quickHits, stories } from "@/data/content";
import { absoluteUrl, siteFeatures } from "@/lib/site";

export default function sitemap(): MetadataRoute.Sitemap {
  const staticRoutes: MetadataRoute.Sitemap = [
    { url: absoluteUrl("/"), changeFrequency: "daily", priority: 1 },
    { url: absoluteUrl("/latest"), changeFrequency: "hourly", priority: 0.9 },
    ...(siteFeatures.quickHits && quickHits.some((video) => Boolean(video.video))
      ? [{ url: absoluteUrl("/quick-hits"), changeFrequency: "daily" as const, priority: 0.8 }]
      : []),
